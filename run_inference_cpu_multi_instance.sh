...
### reference
###   https://github.com/XiaobingSuper/fairseq/tree/master/examples/translation
###
...
for i in $(seq 1 $LAST_INSTANCE); do
    ...
    numactl --physcpubind=$start_core_i-$end_core_i --membind=$numa_node_i fairseq-generate $DATA \
        --path $MODEL --batch-size $BATCH_SIZE --beam 5 --remove-bpe \
        --cpu $ARGS --quiet 2>&1 | tee $LOG_i &
done
...
numactl --physcpubind=$start_core_0-$end_core_0 --membind=$numa_node_0 fairseq-generate $DATA \
        --path $MODEL --batch-size $BATCH_SIZE --beam 5 --remove-bpe \
        --cpu $ARGS --quiet 2>&1 | tee $LOG_0
